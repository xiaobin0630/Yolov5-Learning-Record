...
class YoloDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mosaic and self.rand() < self.mosaic_pro and self.epoch_now < self.epoch_length * self.special_aug_ratio:
            ...
            image,box = self.get_random_data(self.annotation_lines[index],self.input_shape,random=self.trian)
            ...
            for t,ratio in enumerate(max_ratios):
                ...
                for k,mask in enumerate(self.anchors_mask[l]):
                    ...
                    for offset in offsets:
                        ...
                        y_true[l][k, local_j, local_j,c + 5] = 1
                        # 获取当前先验框的最好比列
    ...
    def merge_bboxes(self,bboxes,cutx,cuty):
        ...
        for i in range(len(bboxes)):
            ...
            for box in bboxes[i]:
                ...
                if i == 2:
                    if y2 < cuty or x2 < cutx: # 右下角的坐标比最小值坐标都还小,不要了
                        continue
                    if y2 >= cuty and y1 <= cuty: # y1与y2最小值为cuty 所以将y1设置为cuty
                        y1 = cuty
                    if x2 >= cutx and x1 <= cutx: # 同上
                        x1 = cutx
                # 第四张图片 :cuty, cutx:
                if i == 3:
                    if y1 > cuty or x2 < cutx: # y的最大值为cuty x的的最小值为cutx y1大于cuty或者x2小于cutx，就不要了
                        continue
                    if y2 >= cuty and y1 <= cuty: # y2大于了cuty 而y1还在图内,所以将出界的y2设置为cuty
                        y2 = cuty
                    if x2 >= cutx and x1 <= cutx:# x2在图内区域，x1出界了,所以将x1设为最小值cutx
                        x1 = cutx
                # 吧对应图片对应的框保存在列表里
                temp_box.append(x1)
                temp_box.append(y1)
